...
<body>
  ...
  <header class="bg-white shadow">
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <h1 class="text-3xl font-bold tracking-tight text-gray-900">Kelola Siswa</h1>
    </div>
  </header>
      ...
      <div
        ...
        <div
          ...
          <div class="w-full md:w-1/2">
            <form class="flex items-center" action="/admin/data-siswa" method="GET">
          ...
          <div
            ...
            <?php require_once __DIR__ . '/Modal-Masal.php'; ?>
          ...
          <table
            ...
            <thead
              ...
              <tr>
                ...
                <th scope="col" class="px-4 py-3">DU/DI</th>
            ...
            <tbody>
              <?php foreach ($students as $student): ?>
                <tr class="border-b dark:border-gray-700">
                  ...
                  <td class="px-4 py-3"><?= htmlspecialchars($student['prodi']); ?></td>
                  <td class="px-4 py-3"><?= htmlspecialchars($student['username']); ?></td>
                  <td class="px-4 py-3"><?= htmlspecialchars($student['nama']); ?></td>
                  <td class="px-4 py-3"><?= htmlspecialchars($student['kelamin']); ?></td>
                  <td class="px-4 py-3"><?= htmlspecialchars($student['dudi']); ?></td>
                  ...
                  <td class="px-4 py-3 flex items-center justify-end">
                    ...
                    <div
                      ...
                      <ul
                        ...
                        <li>
                          <a
                            href="/admin/show-siswa?id=<?= htmlspecialchars($student['nis']); ?>"
                            class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Show</a>
                        </li>
                        <li>
                          <form action="/admin/data-siswa" method="POST" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white" onsubmit="return confirm('Apakah Anda yakin ingin mereset akses siswa NIS 12345 nama John Doe? Username akan kembali menjadi 12345 dan password akan menjadi password12345! Tindakan ini tidak dapat dibatalkan.');">
                      ...
                      <form action="/admin/data-siswa" method="POST" class="block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data siswa NIS <?= htmlspecialchars($student['nis']); ?>, nama <?= htmlspecialchars($student['nama']); ?>? Tindakan ini tidak dapat dibatalkan.');">
                      ...
                      <!-- <div class="py-1">
                        ...
                        <a
                          href="/admin/data-siswa?id=<?= htmlspecialchars($student['nis']); ?>"
                          class="block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white" onclick="return confirm('Apakah Anda yakin ingin menghapus data siswa NIS <?= htmlspecialchars($student['nis']); ?>, nama <?= htmlspecialchars($student['nama']); ?>? Tindakan ini tidak dapat dibatalkan.')">Delete</a>
                      </div> -->
                    </div>
        ...
        <nav class="flex flex-col items-center py-4 mb-8">
          ...
          <nav aria-label="Page navigation example">
            <ul class="inline-flex -space-x-px text-sm">
              <!-- Tombol Previous -->
              <li>
                <a href="<?= $currentPage > 1 ? '/admin/data-siswa?page=' . ($currentPage - 1) . '&search=' . htmlspecialchars($search) : '#' ?>"
              ...
              if ($totalPages <= 2) {
                ...
                for ($page = 1; $page <= $totalPages; $page++) {
                  echo '<li>
                    <a href="/admin/data-siswa?page=' . $page . '&search=' . htmlspecialchars($search) . '"
